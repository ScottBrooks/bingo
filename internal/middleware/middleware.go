...
func BundlerWithConfig(cfg BundlerConfig) echo.MiddlewareFunc {
	...
	result := api.Build(api.BuildOptions{
		//		Banner:            map[string]string{"generated": `/* generated by github.com/wolfeidau/echo-esbuild-middleware */`},
		Bundle:            true,
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			log.Ctx(c.Request().Context()).Info().Str("path", c.Path()).Bool("ok", ok).Msg("got a file")
...
func outputFilesToMap(outputFiles []api.OutputFile) map[string][]byte {
	...
	for _, f := range outputFiles {
		...
		assetPath := fmt.Sprintf("/%s", filepath.Base(f.Path))
